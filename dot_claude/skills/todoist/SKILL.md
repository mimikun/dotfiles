...
**中断そのものは失敗ではない。** 判定は原因で1回だけ分岐する。

- **一時的な原因**（体調、突発の予定、割り込み）
  → **失敗ではない。再開するだけ。** 次に着手した時点で継続として扱う
- **同じ条件が揃えばまた止まる原因**
  → **仕組みの側の問題。** タスクの切り方を疑い、導出手順の3
  （必須 / オプショナルの判定）へ戻す

**「連続して回せているか」を完了条件に入れない。** 入れた瞬間、一度の中断で
それまで全部が失敗になり、再開の敷居が上がる。連続性が要るタスクなら、
それは終了条件ではなく回数か期間で切る（分岐B）。

**再開したとき「遅れを取り返す」を持ち込まない。** 止まっていた分を積み増すと
1歩が大きくなり、次の中断で同じことが起きる。**1歩の大きさは中断の前後で変えない。**
