...
- **境界:** 破壊的・不可逆な操作の前は、それでも確認する。
  **対象の一覧は `rules/general.md` の「🚫 絶対禁止」にある。**
...
- **日付を含む提案を出す前に、その日付で Todoist を1回見る。**
  `find-tasks-by-date` を使う（日付を指定するので出力が切れない）。
  **カレンダーを見たかどうかは関係ない。** カレンダーだけ見て提案すると、
  Todoist に既に組んである予定を「新しい案」として出すことになる。
  2026-09-09 に2回起きた（`mimikun.agent-system` の `records/M04`）。

**タスクを作る / 書き換えるときの手順は `todoist` スキルにある。**
終了条件の定義、判定の基準、A / B / C の分岐、記入様式、途中で切れたときの1行、
`projectId` の注意 — すべてあちら。**ここに写さないこと。**
この節が持つのは上の4つ（同期・件数検証・`no-ai`・提案前の確認）だけ。
...
**ここが持つのは「起票するかどうか」の判定だけ。書き方の手順は持たない。**
判定は他のリポジトリで発火するので、そのとき読まれるここに無いと意味がない。

- **🟢 は記録しない。** この仕組みを殺すのは量であって、記入漏れではない。
- **単位は1つの問題。** セッション単位・日付単位では起こさない。
- **顧客や雇用主に関する詳細を、あのリポジトリに持ち込まない。** 問題を抽象化するか、
  記録自体を見送る。個人リポジトリである。

**起票すると決めたら、手順は向こうにある。**
リポジトリは `~/ghq/github.com/mimikun/mimikun.agent-system/`。
**まず `handover.md` を読むこと** — 起票の手順(§0.6)、期限の見かた、
この仕組みが腐らないようにするルールが全部そこにある。**ここに写さないこと。**
...
**名前からは判定できない。命名規則が同じでも公開・非公開は揃っていない。**

```
gh repo view <owner/repo> --json visibility -q .visibility
```

- **引数を省くと cwd のリポジトリを見る。** 空文字を渡すと**別のリポジトリの答えが
  何食わぬ顔で返る**（2026-08-09 に踏んだ。2件を取り違えた）
- **リモートが無いリポジトリもある。** その場合 `gh` は何も返さない。
  **そこへ書いてもバックアップにはならない**ので、置き場として数えない
- **確認した結果をここに書き並べない。** 一覧は公開側に置いた時点で、
  **非公開リポジトリの存在そのものを漏らす**。都度 `gh` で見ること

**可視性と「存在を明かしてよいか」は別の軸。** 非公開リポジトリの中には、
**名前を出すこと自体が不可**のものがある（中身ではなく、あることが知られては困る）。

- **既定は「公開側にリポジトリ名を書かない」。**
- **例外は、本人が明示的に「存在を知られてよい」と決めたものだけ。**
  現時点で該当するのは **`mimikun.agent-system`**（記録リポジトリ）。
  だからこのファイルや `rules/` から名指しで参照してよい
- **決まっていないものは書かない。** 判断は書く前に一度やること。
  **書いた後では戻せない** — 公開リポジトリへ push した時点で、
  ブランチを消しても SHA を知っていればしばらく読める
