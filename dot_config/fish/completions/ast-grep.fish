...
complete -c ast-grep -n "__fish_ast_grep_using_subcommand run" -l json -d 'Output matches in structured JSON ' -r -f -a "{pretty\t'Prints the matches as a pretty-printed JSON array, with indentation and line breaks. This is useful for human readability, but not for parsing by other programs. This is the default value for the `--json` option',stream\t'Prints each match as a separate JSON object, followed by a newline character. This is useful for streaming the output to other programs that can read one object per line',compact\t'Prints the matches as a single-line JSON array, without any whitespace. This is useful for saving space and minimizing the output size'}"
...
complete -c ast-grep -n "__fish_ast_grep_using_subcommand run" -l inspect -d 'Inspect information for file/rule discovery and scanning' -r -f -a "{nothing\t'Do not show any tracing information',summary\t'Show summary about how many files are scanned and skipped',entity\t'Show per-file/per-rule tracing information'}"
...
complete -c ast-grep -n "__fish_ast_grep_using_subcommand run" -l heading -d 'Controls whether to print the file name as heading' -r -f -a "{auto\t'Print heading for terminal tty but not for piped output',always\t'Always print heading regardless of output type',never\t'Never print heading regardless of output type'}"
...
complete -c ast-grep -n "__fish_ast_grep_using_subcommand scan" -l report-style -r -f -a "{rich\t'Output a richly formatted diagnostic, with source code previews',medium\t'Output a condensed diagnostic, with a line number, severity, message and notes (if any)',short\t'Output a short diagnostic, with a line number, severity, and message'}"
...
complete -c ast-grep -n "__fish_ast_grep_using_subcommand scan" -l no-ignore -d 'Do not respect hidden file system or ignore files (.gitignore, .ignore, etc.)' -r -f -a "{hidden\t'Search hidden files and directories. By default, hidden files and directories are skipped',dot\t'Don\'t respect .ignore files. This does *not* affect whether ast-grep will ignore files and directories whose names begin with a dot. For that, use --no-ignore hidden',exclude\t'Don\'t respect ignore files that are manually configured for the repository such as git\'s \'.git/info/exclude\'',global\t'Don\'t respect ignore files that come from "global" sources such as git\'s `core.excludesFile` configuration option (which defaults to `$HOME/.config/git/ignore`)',parent\t'Don\'t respect ignore files (.gitignore, .ignore, etc.) in parent directories',vcs\t'Don\'t respect version control ignore files (.gitignore, etc.). This implies --no-ignore parent for VCS files. Note that .ignore files will continue to be respected'}"
...
complete -c ast-grep -n "__fish_ast_grep_using_subcommand scan" -l json -d 'Output matches in structured JSON ' -r -f -a "{pretty\t'Prints the matches as a pretty-printed JSON array, with indentation and line breaks. This is useful for human readability, but not for parsing by other programs. This is the default value for the `--json` option',stream\t'Prints each match as a separate JSON object, followed by a newline character. This is useful for streaming the output to other programs that can read one object per line',compact\t'Prints the matches as a single-line JSON array, without any whitespace. This is useful for saving space and minimizing the output size'}"
...
complete -c ast-grep -n "__fish_ast_grep_using_subcommand scan" -l inspect -d 'Inspect information for file/rule discovery and scanning' -r -f -a "{nothing\t'Do not show any tracing information',summary\t'Show summary about how many files are scanned and skipped',entity\t'Show per-file/per-rule tracing information'}"
