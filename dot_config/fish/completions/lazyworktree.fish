...
function __fish_lazyworktree_no_subcommand --description 'Test if there has been any subcommand yet'
    for i in (commandline -opc)
        if contains -- $i create rename delete list ls exec note describe help h completion
            return 1
        end
    end
    return 0
...
complete -c lazyworktree -n '__fish_seen_subcommand_from create' -f -l from-branch -s branch -r -d 'Create worktree from branch (defaults to current branch)'
...
complete -c lazyworktree -n '__fish_seen_subcommand_from create' -f -l exec-mode -r -d 'Shell invocation mode for --exec: direct|shell|login-shell (default: login-shell)'
...
complete -c lazyworktree -n '__fish_seen_subcommand_from create' -f -l note -r -d 'Set a note on the new worktree'
complete -c lazyworktree -n '__fish_seen_subcommand_from create' -f -l note-file -r -d 'Read note from file (use \'-\' for stdin)'
complete -c lazyworktree -n '__fish_seen_subcommand_from create' -f -l description -r -d 'Set a description on the new worktree'
complete -c lazyworktree -n '__fish_seen_subcommand_from create' -f -l tags -r -d 'Comma-separated tags for the new worktree'
complete -c lazyworktree -n '__fish_seen_subcommand_from create' -f -l json -d 'Output result as JSON'
...
complete -c lazyworktree -n '__fish_seen_subcommand_from rename' -f -l json -d 'Output result as JSON'
...
complete -c lazyworktree -n '__fish_seen_subcommand_from delete' -f -l json -d 'Output result as JSON'
...
complete -c lazyworktree -n '__fish_seen_subcommand_from list ls' -f -l no-agent -d 'Skip agent session data in JSON output (faster for scripting)'
...
complete -c lazyworktree -n '__fish_seen_subcommand_from exec' -f -l json -d 'Output result as JSON; command stdout/stderr is redirected to stderr'
...
complete -c lazyworktree -n '__fish_seen_subcommand_from note; and __fish_seen_subcommand_from show' -f -l json -d 'Output note as JSON including metadata'
...
complete -x -c lazyworktree -n '__fish_lazyworktree_no_subcommand' -a 'describe' -d 'Describe the CLI structure as JSON for machine-readable introspection'
complete -c lazyworktree -n '__fish_seen_subcommand_from describe' -f -l all -d 'Describe all commands and their flags'
complete -c lazyworktree -n '__fish_seen_subcommand_from describe' -f -l help -s h -d 'show help'
complete -x -c lazyworktree -n '__fish_seen_subcommand_from describe; and not __fish_seen_subcommand_from help h' -a 'help' -d 'Shows a list of commands or help for one command'
