...
complete -c uvx -s i -l index-url -d 'The URL of the Python package index (by default: <https://pypi.org/simple>)' -r
complete -c uvx -l extra-index-url -d 'Extra URLs of package indexes to use, in addition to `--index-url`' -r
...
complete -c uvx -l color -d 'Control colors in output' -r -f -a "{auto\t'Enables colored output only when the output is going to a terminal or TTY with support',always\t'Enables colored output regardless of the detected environment',never\t'Disables colored output'}"
