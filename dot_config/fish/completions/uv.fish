...
function __fish_uv_global_optspecs
	string join \n q/quiet v/verbose no-color color= native-tls no-native-tls offline no-offline python-preference= python-fetch= preview no-preview isolated show-settings no-progress directory= n/no-cache cache-dir= config-file= no-config h/help V/version
end
...
complete -c uv -n "__fish_uv_needs_command" -f -a "tool" -d 'Run and manage executable Python packages'
complete -c uv -n "__fish_uv_needs_command" -f -a "python" -d 'Manage Python installations'
complete -c uv -n "__fish_uv_needs_command" -f -a "init" -d 'Initialize a project'
complete -c uv -n "__fish_uv_needs_command" -f -a "run" -d 'Run a command in the project environment'
complete -c uv -n "__fish_uv_needs_command" -f -a "sync" -d 'Sync the project\'s dependencies with the environment'
complete -c uv -n "__fish_uv_needs_command" -f -a "lock" -d 'Resolve the project requirements into a lockfile'
complete -c uv -n "__fish_uv_needs_command" -f -a "add" -d 'Add one or more packages to the project requirements'
complete -c uv -n "__fish_uv_needs_command" -f -a "remove" -d 'Remove one or more packages from the project requirements'
complete -c uv -n "__fish_uv_needs_command" -f -a "tree" -d 'Display the dependency tree for the project'
