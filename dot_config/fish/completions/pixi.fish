...
complete -c pixi -n "__fish_pixi_using_subcommand publish" -l to -d 'The target channel URL to publish packages to' -r
...
complete -c pixi -n "__fish_pixi_using_subcommand publish" -l no-install -d 'Don\'t modify the environment, only modify the lock-file'
complete -c pixi -n "__fish_pixi_using_subcommand publish" -l no-lockfile-update -d 'DEPRECATED: use `--frozen` `--no-install`. Skips lock-file updates'
complete -c pixi -n "__fish_pixi_using_subcommand publish" -l frozen -d 'Install the environment as defined in the lockfile, doesn\'t update lockfile if it isn\'t up-to-date with the manifest file'
complete -c pixi -n "__fish_pixi_using_subcommand publish" -l locked -d 'Check if lockfile is up-to-date before installing the environment, aborts when lockfile isn\'t up-to-date with the manifest file'
complete -c pixi -n "__fish_pixi_using_subcommand publish" -l as-is -d 'Shorthand for the combination of --no-install and --frozen'
